...
function PluginSettings(props) {
  ...
  const plugins = useMemo(() => {
    const all = service?.data?.plugins || [];
    return [ ...all ].sort((a, b) => (b.installs || 0) - (a.installs || 0));
  }, [service]);
  ...
  // Changing the plugin changes which series the widget reads, so the old one
  // has to go rather than sit under the new numbers.
  const onReset = async () => {
        ...
        <ServiceAccordion expanded={expanded === 'servicePanel'}
          ...
          <FormControl>
            <Select value={slug} onChange={(ev) => setSlug(ev.target.value)}>
              {plugins.map((x) => <MenuItem key={x.slug} value={x.slug}>{x.name}</MenuItem>)}
            </Select>
            <FormHelperText>Select the plugin.</FormHelperText>
          </FormControl>
        </ServiceAccordion>
